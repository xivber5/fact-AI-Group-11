...
# def swin_base_patch16_224(pretrained=False, **kwargs):
#     """
#     Initialize a Swin Transformer (base configuration) with patch size 4 and window size 7 for 224x224 input images.
#     Args:
#         pretrained (bool): Whether to load pretrained weights.
#         **kwargs: Additional arguments for the SwinTransformer model.
#     Returns:
#         SwinTransformer: The initialized Swin Transformer model.
#     """
#     model = VisionTransformer(
#         patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     model.default_cfg = _cfg()
#     if pretrained:
#         pretrained_model_url = "https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224.pth"
#         model.load_pretrained(pretrained_model_url)
#     return model
...
def swin_base_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        for key in model_state_dict.keys():
            if key in pretrained_state_dict and model_state_dict[key].shape == pretrained_state_dict[key].shape:
